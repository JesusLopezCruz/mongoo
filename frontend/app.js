...
function mostrarProductos(productos) {
  ...
  productos.forEach(producto => {
    ...
    const divProducto = document.createElement('table');
    //divProducto.className = 'tables';
...
/****  listado de productos  *************************** */
    divProducto.innerHTML = `
            <div class="row card-body">
                <div class="col">
                  <button class="btn" onclick="agregarAlCarrito('${producto._id}', '${producto.nombre}', ${producto.precio})" ${producto.stock === 0 ? 'disabled' : ''}>  🛒  </button> 
                  <button class="btn" onclick="editarProducto('${producto._id}')"> ✏️ </button>
                  <button class="btn" onclick="eliminarProducto('${producto._id}', '${producto.nombre}')"> 🗑️ </button> 
                  </div>
                  <div class="col">${producto.nombre}</div>
                <div class="col">${producto.precio.toFixed(2)}</div>
...
function mostrarPedidos(pedidos) {
  ...
  pedidos.forEach(pedido => {
    ...
    divPedido.innerHTML = `
      ...
      <div class="card-body">
              <h6 class="text-end p-1 estado-${pedido.estado}"> ${pedido.estado}</h6>
              <p class="text-end">${new Date(pedido.createdAt).toLocaleDateString('es-ES', {
            year: 'numeric',  month: '2-digit',  day: '2-digit'})}</p>
   
          <h6>${pedido.cliente}</h6>
          <hr>
        <div class="row card-body">
                <p class="small col">${productosHTML}</p>
                <p class="small col text-end"><strong>€${pedido.total.toFixed(2)} </strong></p>
        </div>
        <div class="row">
          <button class="btn col" onclick="cambiarEstadoPedido('${pedido._id}', 'completado')" 
                  ${pedido.estado === 'completado' ? 'disabled' : ''}>   ✅    </button>
          <button class="btn col" onclick="cambiarEstadoPedido('${pedido._id}', 'cancelado')"
                  ${pedido.estado === 'cancelado' ? 'disabled' : ''}>   ❌     </button>
          <button class="btn col" onclick="eliminarPedido('${pedido._id}', '${pedido.cliente}')">🗑️</button>
        </div>        
